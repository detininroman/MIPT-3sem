...
int sender (char* filename)
{
	...
	if (fd_main_fifo == -1) {
		perror ("Sender: Can't open main fifo!\n");
		exit (EXIT_FAILURE);
	}
...
int reciever (int output)
{
	...
	if (fd_ID_fifo == -1) {
		perror ("Reciever: Can't open current fifo!\n");
        exit (EXIT_FAILURE);
	}
    int flag = fcntl (fd_ID_fifo, F_GETFL, 0);
